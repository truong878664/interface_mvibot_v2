...
@section('content')
    <div class="lg:flex w-[calc(100%_-_10px)] h-[calc(100%_-_10px)] m-2 overflow-auto border">
        <div class="h-2/3 lg:h-full lg:w-2/3 relative">
            ...
            <div class="absolute h-[30px]  m-4  flex">
                <label class=" text-[#fff]px-2 rounded-md z-[1000] bg-main flex text-2xl text-[#fff] font-bold">
                    <select name="" id="robot-mapping" class="bg-transparent outline-none ">
                        <option value="" class="text-[#000]">select robot</option>
                        @foreach ($robotSlam as $item)
                            <option value="{{ $item['name_seri'] }}" class="text-[#000]">{{ $item['name_seri'] }}</option>
                        @endforeach
                    </select>
                </label>
                <div class="ml-[30px]">
        ...
        <div
            ...
            <div class=" flex h-full items-center w-full justify-end lg:flex-col relative">
                <div class="absolute top-[50px] left-0 w-[90%]">
                    <span id="" class="rs-label absolute -right-2 top-0 translate-x-full -translate-y-1/2 text-2xl font-bold leading-[30px] label-range-topic">16</span>
                    <input id="" class="rs-range w-full input-range-topic" type="range" value="100" min="0" max="100">
                </div>
                <div class="absolute top-0 right-0 mt-4 flex">
                    <button data-type="joystick" data-active="active" class="choose-joystick-btn btn w-[40px] text-[#fff] h-[30px] bg-[#ccc] data-[active=active]:bg-[#a8a8a8] data-[active=active]:text-[#ccc] mr-0 rounded-l-md"><i class="fa-solid fa-gamepad"></i></button>
                    <button data-type="move-action" data-active="" class="choose-joystick-btn btn w-[40px] text-[#fff] h-[30px] bg-[#ccc] data-[active=active]:bg-[#a8a8a8] data-[active=active]:text-[#ccc] ml-0 rounded-r-md"><i class="fa-solid fa-arrows-up-down-left-right"></i></button>
                </div>
