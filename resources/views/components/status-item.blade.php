    ...
    <div class="status-item">
        ...
        @elseif($robotType === 'module_gpio')
            <div class="flex flex-col w-full">
                <div class="flex flex-col w-full">
                    <span class="font-bold ">Input gpio</span>
                    <div class="w-full flex justify-between ">
                        @foreach (explode(',', $moduleIn) as $index => $io)
                            <div
                                class="text-4xl {{ (int) $io === 1 ? 'text-green-400' : 'text-red-500/90' }} io_module relative">
                                <i class="fa-solid fa-circle"></i>
                                <span
                                    class="absolute font-bold top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[#fff] text-xl">{{ $index }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="flex flex-col w-full mt-8">
                    <span class="font-bold ">Output gpio</span>
                    <div class="w-full flex justify-between ">
